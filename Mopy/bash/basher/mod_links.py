...
__all__ = [u'Mod_FullLoad', u'Mod_CreateDummyMasters', u'Mod_OrderByName',
           ...
           u'Mod_UndeleteRefs', u'Mod_AddMaster', u'Mod_CopyToEsmp',
           u'Mod_DecompileAll', u'Mod_FlipEsm', u'Mod_FlipEsl',
           u'Mod_FlipMasters', u'Mod_SetVersion', u'Mod_ListDependent',
           u'Mod_JumpToInstaller', u'Mod_Move']
...
class _Mod_Patch_Update(_Mod_BP_Link):
    ...
    def _Execute(self):
        ...
        mods_prior_to_patch = load_order.cached_lower_loading_espms(
            self._selected_item)
        if self.doCBash or bass.settings[u'bash.CBashEnabled']:
...
class _Import_Export_Link(AppendableLink):
    ...
    def _append(self, window):
        ...
        try:
            # Check if all record types required by this parser not exist for
            # this game and are not supported for loading
            rec_types = bosh.MreRecord.type_class
...
class Mod_ItemData_Import(_Mod_Import_Link): # CRUFT
    ...
    continueKey = u'bash.itemdata.import.continue'
...
class MasterList_CleanMasters(AppendableLink, ItemLink): # CRUFT
    ...
    def _append(self, window): return bass.settings[u'bash.CBashEnabled']
