...
#------------------------------------------------------------------------------
class AAssortedTweak_ArmorPlayable(_AAssortedTweak):
...
class _PSeffWeightTweak(AMultiTweakItem_Weight, _AssortPTweak):
    ...
    def wants_record(self, record):
        return super(_PSeffWeightTweak, self).wants_record(
            record) and not (b'SEFF', 0) in record.getEffects()
...
class AAssortedTweak_PotionWeightMinimum(AMultiTweakItem_Weight):
    ...
    tweak_choices = [(u'1', 1), (u'2', 2), (u'3', 3), (u'4', 4),
                     (_(u'Custom'), 0.0)]
    tweak_log_msg = _(u'Potions Reweighed: %(total_changed)d')
...
class AssortedTweak_PotionWeightMinimum(
    _PSeffWeightTweak, AAssortedTweak_PotionWeightMinimum): pass
class CBash_AssortedTweak_PotionWeightMinimum(
    _CSeffWeightTweak, AAssortedTweak_PotionWeightMinimum):
...
class AAssortedTweak_SetCastWhenUsedEnchantmentCosts(_AAssortedTweak):
    ...
    def wants_record(self, record):
        if not record.itemType in (1, 2): return False
